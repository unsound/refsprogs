...
if [ "$(echo `lsb_release -i | cut -d ':' -f 2-`)" == "Debian" ]; then
    case "$(echo `lsb_release -c | cut -d ':' -f 2-`)" in
	"buster")
	    archs="alpha hppa ia64 m68k powerpc ppc64 riscv64 sh4 sparc64 x32"
	    ;;
	"forky")
	    archs="hppa loong64 m68k powerpc ppc64 sh4 sparc64 x32"
	    ;;
	*)
	    ;;
    esac
